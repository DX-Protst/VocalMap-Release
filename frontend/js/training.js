...
function updateCustomRange() {
    ...
    if (lowest >= highest) {
        alert(t('train.lowest_highest_alert', "最高音必须高于最低音！"));
...
function proceedToLevelSelection() {
    ...
    document.getElementById('displaySelectedRange').innerText = t('train.' + currentSelectedRange, RANGE_NAMES[currentSelectedRange]);
}
...
function startTraining() {
    if (!isRunning || ws.readyState !== WebSocket.OPEN) {
        if (typeof showToast === 'function') {
            showToast(t('train.mic_warn_title', "全息舱检测警告"), t('train.mic_warn_msg', "检测到系统引擎尚未就绪。请先点击右上角【连接后端并启动】按钮，打通声学网关！"), "warning");
        } else {
            alert(t('train.mic_warn_msg', "请先点击右上角的【连接后端并启动】引擎！"));
        }
        return;
    }
...
window.addEventListener('languagechanged', (e) => {
    if (currentSelectedRange) {
        const displayEl = document.getElementById('displaySelectedRange');
        if (displayEl) {
            displayEl.innerText = t('train.' + currentSelectedRange, RANGE_NAMES[currentSelectedRange]);
        }
    }
    if (currentSelectedLevel && activeTrainingSequence) {
        const levelNameEl = document.getElementById('trainingActiveLevelName');
        if (levelNameEl) {
            const activeLevelText = t('train.level_' + currentSelectedLevel, LEVEL_NAMES[currentSelectedLevel]);
            levelNameEl.innerText = t('train.active_level_prefix', '正在进行: 关卡 ') + `${currentSelectedLevel} - ${activeLevelText}`;
        }
    }
});
