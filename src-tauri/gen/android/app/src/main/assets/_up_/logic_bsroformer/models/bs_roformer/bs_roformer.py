...
class BSRoformer(Module):
    ...
    def forward(
        ...
        for i, transformer_block in enumerate(self.layers):
            ...
            if self.use_torch_checkpoint:
                # Chunking to save VRAM on standard attention
                chunk_size_bf = 16
                out_chunks = []
                for i in range(0, x.shape[0], chunk_size_bf):
                    chunk = x[i:i+chunk_size_bf]
                    out_chunks.append(checkpoint(time_transformer, chunk, use_reentrant=False))
                ...
                for i in range(0, x.shape[0], chunk_size_bf):
                    chunk = x[i:i+chunk_size_bf]
                    out_chunks.append(time_transformer(chunk))
                x = torch.cat(out_chunks, dim=0)
            ...
            if self.use_torch_checkpoint:
                chunk_size_bt = 16
                out_chunks = []
                for i in range(0, x.shape[0], chunk_size_bt):
                    chunk = x[i:i+chunk_size_bt]
                    out_chunks.append(checkpoint(freq_transformer, chunk, use_reentrant=False))
                x = torch.cat(out_chunks, dim=0)
            else:
                chunk_size_bt = 16
                out_chunks = []
                for i in range(0, x.shape[0], chunk_size_bt):
                    chunk = x[i:i+chunk_size_bt]
                    out_chunks.append(freq_transformer(chunk))
                x = torch.cat(out_chunks, dim=0)
